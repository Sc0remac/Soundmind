...
export default function AdminBodyParts() {
  ...
  async function remove(id: string) {
    const { data: session } = await supabase.auth.getSession();
    const token = session.session?.access_token;
    await fetch("/api/admin/body-parts", {
      method: "DELETE",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold">Global Body Parts</h2>
      <div className="flex gap-2">
        <input className="border rounded p-2" placeholder="New body part…" value={name} onChange={(e) => setName(e.target.value)} />
